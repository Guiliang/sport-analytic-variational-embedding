...
def gathering_running_and_run(dir_game, config, player_id_cluster_dir, data_store,
    ...
    initial_prior_mu_train = np.zeros(shape=[config.Learn.batch_size, config.Arch.CVRNN.latent_dim])
    previous_prior_mu_train = initial_prior_mu_train
    initial_prior_sigma_train = np.zeros(shape=[config.Learn.batch_size, config.Arch.CVRNN.latent_dim])
    ...
    while True:
        ...
        previous_prior_mu_train_seq = np.zeros(shape=[len(batch_return),
                                                      config.Learn.max_seq_length,
                                                      config.Arch.CVRNN.latent_dim])
        previous_prior_mu_train_seq[:, 0, :] = initial_prior_mu_train
        previous_prior_sigma_train_seq = np.zeros(shape=[len(batch_return),
                                                         config.Learn.max_seq_length,
                                                         config.Arch.CVRNN.latent_dim])
        ...
        if config.Learn.predict_target == 'PlayerLocalId':
            input_data_t0 = np.concatenate([np.asarray(player_id_t0_batch),
                                            ...
                                            previous_prior_mu_train_seq,
                                            previous_prior_sigma_train_seq,
                                            train_mask], axis=2)
...
def run():
    ...
    icehockey_tpp_cvrnn_config_path = "../environment_settings/icehockey_cvrnn{0}_config.yaml".format(predicted_target)
